...
def pcaps2csi(pcaps):
    ...
    random.shuffle(csi_sala_vazia)

    #balanceando
    #csi_parada = csi_parada[:len(csi_movimento)]
    csi_presenca, csi_sala_vazia = balancear_listas(csi_presenca, csi_sala_vazia)
    
    ###### 
    print('tamanho da lista csi_presenca')
    print(len(csi_presenca))
    print('tamanho da lista csi_sala_vazia')
    print(len(csi_sala_vazia))

    # Concatenando as duas listas em uma única lista de dados
    csi_total = csi_presenca + csi_sala_vazia
    # Criando rótulos para os dados (1 para dados com pessoa, 0 para dados sem pessoa)
    labels = [1] * len(csi_presenca) + [0] * len(csi_sala_vazia)
    #print(labels)
