...
def path_to_csi(lista_dos_path):
    ...
    for path in lista_dos_path:
        ...
        # Divide em janelas de 'janela' amostras (66 amostras por 2 segundos)
        num_janelas = csi.shape[1] // 16  # Número total de janelas
        for i in range(num_janelas):
            csi_janela = csi[:, i * 16:(i + 1) * 16, :]
            lista_csi.append(csi_janela)
...
def pcaps2csi(pcaps):
    ...
    random.shuffle(csi_movimento)
    random.shuffle(csi_parada)

    #balanceando
    #csi_parada = csi_parada[:len(csi_movimento)]
    csi_movimento, csi_parada = balancear_listas(csi_movimento, csi_parada)
    
    ###### 
    print('tamanho da lista csi_movimento')
    print(len(csi_movimento))
    print('tamanho da lista csi_parada')
    print(len(csi_parada))

    # Concatenando as duas listas em uma única lista de dados
    csi_total = csi_movimento + csi_parada
    # Criando rótulos para os dados (1 para dados com pessoa, 0 para dados sem pessoa)
    labels = [1] * len(csi_movimento) + [0] * len(csi_parada)
    #print(labels)
...
if __name__=="__main__":
    ...
    val_loader = DataLoader(val_dataset, batch_size=12,num_workers=1)
    torch.save(val_loader, "val_loader.pt")
    test_loader = DataLoader(test_dataset, batch_size=12,num_workers=1)
