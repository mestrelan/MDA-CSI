...
def pcaps2csi(pcaps):
    ...
    csi_presenca=path_to_csi(pcaps_presenca)
    csi_deitar_levantar=path_to_csi(pcaps_deitar_levantar)
    ...
    random.shuffle(csi_deitar_levantar)

    #balanceando
    #csi_parada = csi_parada[:len(csi_presenca)]
    csi_presenca, csi_deitar_levantar = balancear_listas(csi_presenca, csi_deitar_levantar)
    
    ###### 
    print('tamanho da lista csi_presenca')
    print(len(csi_presenca))
    print('tamanho da lista csi_deitar_levantar')
    print(len(csi_deitar_levantar))

    # Concatenando as duas listas em uma única lista de dados
    csi_total = csi_presenca + csi_deitar_levantar
    # Criando rótulos para os dados (1 para dados com pessoa, 0 para dados sem pessoa)
    labels = [1] * len(csi_presenca) + [0] * len(csi_deitar_levantar)
    #print(labels)
