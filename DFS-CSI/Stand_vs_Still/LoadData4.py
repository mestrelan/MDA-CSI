...
def pcaps2csi(pcaps):
    ...
    csi_naope=path_to_csi(pcaps_sentada)+path_to_csi(pcaps_deitado)
    ...
    random.shuffle(csi_pe)
    random.shuffle(csi_naope)

    #balanceando
    #csi_parada = csi_parada[:len(csi_movimento)]
    csi_pe, csi_naope = balancear_listas(csi_pe, csi_naope)
    
    ###### 
    print('tamanho da lista csi_pe')
    print(len(csi_pe))
    print('tamanho da lista csi_naope')
    print(len(csi_naope))

    # Concatenando as duas listas em uma única lista de dados
    csi_total = csi_pe + csi_naope
    # Criando rótulos para os dados (1 para dados com pessoa, 0 para dados sem pessoa)
    labels = [1] * len(csi_pe) + [0] * len(csi_naope)
    #print(labels)
...
if __name__=="__main__":
    ...
    val_loader = DataLoader(val_dataset, batch_size=4,num_workers=1)
