...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        # Variáveis para calcular acurácia de treino
        total_train = 0
        correct_train = 0
        ...
        for inputs, labels in train_loader:
            ...
            # --- CÁLCULO DA ACURÁCIA DE TREINO (NOVO) ---
        ...
        average_train_accuracy = correct_train / total_train # <--- Média da época
        
        train_losses.append(average_train_loss)
        train_accuracies.append(average_train_accuracy) # <--- Salva na lista
        ...
        print(f'Train Loss: {average_train_loss:.4f}, Train Acc: {average_train_accuracy:.4f}') # <--- Print atualizado
        print(f'Val Loss: {val_loss:.4f}, Val Accuracy: {val_accuracy:.4f}')
    
    epochs = range(1, num_epochs + 1)
    
    # Retorna agora 5 itens em vez de 4
    listas_treinamento = epochs, train_losses, train_accuracies, val_losses, val_accuracies
...
if __name__=="__main__":
    ...
    model = TransformerModel(input_size=48, output_size=1, num_heads=12, num_layers=9).to(device)#num_heads=8, num_layers=6
    ...
    inicio_train_eval=time.time()
    listas_treinamento=train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, 40)
    fim_train_eval=time.time()
    temp_train_eval = fim_train_eval - inicio_train_eval
    ...
    fim_test=time.time()
    temp_test = fim_test - inicio_test
