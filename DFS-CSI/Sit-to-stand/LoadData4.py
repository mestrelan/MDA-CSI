...
def pcaps2csi(pcaps):
    ...
    csi_presenca=path_to_csi(pcaps_presenca)
    csi_sentar_levantar=path_to_csi(pcaps_sentar_levantar)
    ...
    random.shuffle(csi_sentar_levantar)

    #balanceando
    #csi_parada = csi_parada[:len(csi_movimento)]
    csi_presenca, csi_sentar_levantar = balancear_listas(csi_presenca, csi_sentar_levantar)
    
    ###### 
    print('tamanho da lista csi_presenca')
    print(len(csi_presenca))
    print('tamanho da lista csi_sentar_levantar')
    print(len(csi_sentar_levantar))

    # Concatenando as duas listas em uma única lista de dados
    csi_total = csi_presenca + csi_sentar_levantar
    # Criando rótulos para os dados (1 para dados com pessoa, 0 para dados sem pessoa)
    labels = [1] * len(csi_presenca) + [0] * len(csi_sentar_levantar)
    #print(labels)
...
if __name__=="__main__":
    ...
    #batch_size = 4#32
    train_loader = DataLoader(train_dataset, batch_size=24, shuffle=True, num_workers=1)
    torch.save(train_loader, "train_loader.pt")
    val_loader = DataLoader(val_dataset, batch_size=24,num_workers=1)
    torch.save(val_loader, "val_loader.pt")
    test_loader = DataLoader(test_dataset, batch_size=24,num_workers=1)
