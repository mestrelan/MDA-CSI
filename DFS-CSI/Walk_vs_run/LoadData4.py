...
def pcaps2csi(pcaps):
    ...
    random.shuffle(csi_correndo)

    #balanceando
    #csi_parada = csi_parada[:len(csi_movimento)]
    csi_andando, csi_correndo = balancear_listas(csi_andando, csi_correndo)
    
    ###### 
    print('tamanho da lista csi_andando')
    print(len(csi_andando))
    print('tamanho da lista csi_correndo')
    print(len(csi_correndo))

    # Concatenando as duas listas em uma única lista de dados
    csi_total = csi_andando + csi_correndo
    # Criando rótulos para os dados (1 para dados com pessoa, 0 para dados sem pessoa)
    labels = [1] * len(csi_andando) + [0] * len(csi_correndo)
    #print(labels)
