...
def pcaps2csi(pcaps):
    ...
    csi_deitado=path_to_csi(pcaps_deitado)
    csi_naodeitado=path_to_csi(pcaps_sentada)+path_to_csi(pcaps_pe)
    ...
    random.shuffle(csi_deitado)
    random.shuffle(csi_naodeitado)

    #balanceando
    #csi_parada = csi_parada[:len(csi_movimento)]
    csi_deitado, csi_naodeitado = balancear_listas(csi_deitado, csi_naodeitado)
    
    ###### 
    print('tamanho da lista csi_deitado')
    print(len(csi_deitado))
    print('tamanho da lista csi_naodeitado')
    print(len(csi_naodeitado))

    # Concatenando as duas listas em uma única lista de dados
    csi_total = csi_deitado + csi_naodeitado
    # Criando rótulos para os dados (1 para dados com pessoa, 0 para dados sem pessoa)
    labels = [1] * len(csi_deitado) + [0] * len(csi_naodeitado)
    #print(labels)
